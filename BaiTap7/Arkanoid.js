...
const brickColCount = 16;
const brickWidth = 103;
...
function resetAllAnkanGame() {
  buildBricks();
  resetBasicMaterial();
}

function resetBasicMaterial() {
  noPlus = true
  forwardTheBall.y =  waytogo.y - waytogo.bh;
...
function moveBall() {
  ...
  }else if (forwardTheBall.x >= canvas.width){
...
function arkanLoop() {
  ...
  if (level == 7){
    ...
    renderWave()
    drawBricks()
  } else if (forwardTheBall.y >= waytogo.y + waytogo.bh){
    GameOver()
  }else{
    ctx.fillStyle = 'black';
    ctx.fillRect(0, 0, canvas.width, canvas.height);
